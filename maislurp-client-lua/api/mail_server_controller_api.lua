...
function mail_server_controller_api:describe_mail_server_domain(describe_options)
	...
	req:set_body(dkjson.encode(describe_options))
...
function mail_server_controller_api:verify_email_address(verify_options)
	local req = http_request.new_from_uri({
		scheme = self.default_scheme;
		host = self.host;
		port = self.port;
	...
	req:set_body(dkjson.encode(verify_options))
