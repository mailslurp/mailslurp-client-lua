...
function tracking_controller_api:get_all_tracking_pixels(before, page, search_filter, since, size, sort)
	local req = http_request.new_from_uri({
		scheme = self.default_scheme;
		host = self.host;
		port = self.port;
		path = string.format("%s/tracking/pixels?before=%s&page=%s&searchFilter=%s&since=%s&size=%s&sort=%s",
