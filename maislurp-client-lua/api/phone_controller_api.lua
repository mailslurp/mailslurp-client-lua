...
function phone_controller_api:get_phone_numbers(page, size, sort, since, before)
	local req = http_request.new_from_uri({
		scheme = self.default_scheme;
		host = self.host;
		port = self.port;
		path = string.format("%s/phone/numbers?page=%s&size=%s&sort=%s&since=%s&before=%s",
...
function phone_controller_api:test_phone_number_send_sms(phone_number_id, test_phone_number_options)
	local req = http_request.new_from_uri({
		scheme = self.default_scheme;
		host = self.host;
		port = self.port;
