...
function inbox_forwarder_controller_api:get_inbox_forwarders(before, inbox_id, page, search_filter, since, size, sort)
	local req = http_request.new_from_uri({
		scheme = self.default_scheme;
		host = self.host;
		port = self.port;
		path = string.format("%s/forwarders?before=%s&inboxId=%s&page=%s&searchFilter=%s&since=%s&size=%s&sort=%s",
