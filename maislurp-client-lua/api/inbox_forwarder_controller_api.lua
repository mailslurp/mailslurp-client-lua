...
function inbox_forwarder_controller_api:create_new_inbox_forwarder(inbox_id, create_inbox_forwarder_options)
...
function inbox_forwarder_controller_api:get_all_inbox_forwarder_events(page, size, inbox_id, sort)
	local req = http_request.new_from_uri({
		scheme = self.default_scheme;
		host = self.host;
		port = self.port;
		path = string.format("%s/forwarders/events?page=%s&size=%s&inboxId=%s&sort=%s",
			self.basePath, http_util.encodeURIComponent(page), http_util.encodeURIComponent(size), http_util.encodeURIComponent(inbox_id), http_util.encodeURIComponent(sort));
	})
