...
local maislurp-client-lua_page_inbox_forwarder_events = require "maislurp-client-lua.model.page_inbox_forwarder_events"
...
function inbox_forwarder_controller_api:get_all_inbox_forwarder_events(page, size, inbox_id, email_id, sent_id, sort)
	local req = http_request.new_from_uri({
		scheme = self.default_scheme;
		host = self.host;
		port = self.port;
		path = string.format("%s/forwarders/events?page=%s&size=%s&inboxId=%s&emailId=%s&sentId=%s&sort=%s",
			self.basePath, http_util.encodeURIComponent(page), http_util.encodeURIComponent(size), http_util.encodeURIComponent(inbox_id), http_util.encodeURIComponent(email_id), http_util.encodeURIComponent(sent_id), http_util.encodeURIComponent(sort));
	})
	...
	if http_status:sub(1,1) == "2" then
		...
		return maislurp-client-lua_page_inbox_forwarder_events.cast(result), headers
	else
		local body, err, errno2 = stream:get_body_as_string()
		if not body then
			return nil, err, errno2
...
function inbox_forwarder_controller_api:get_forwarder_event(event_id)
	local req = http_request.new_from_uri({
		scheme = self.default_scheme;
		host = self.host;
		port = self.port;
		path = string.format("%s/forwarders/events/%s",
			self.basePath, event_id);
	})
	...
	if http_status:sub(1,1) == "2" then
		...
		return maislurp-client-lua_inbox_forwarder_event_dto.cast(result), headers
	else
		local body, err, errno2 = stream:get_body_as_string()
		if not body then
			return nil, err, errno2
...
function inbox_forwarder_controller_api:get_inbox_forwarder_event(id, event_id)
	local req = http_request.new_from_uri({
		scheme = self.default_scheme;
		host = self.host;
		port = self.port;
		path = string.format("%s/forwarders/%s/events/%s",
			self.basePath, id, event_id);
	})
...
function inbox_forwarder_controller_api:get_inbox_forwarder_events(id, page, size, sort)
	local req = http_request.new_from_uri({
		scheme = self.default_scheme;
		host = self.host;
		port = self.port;
		path = string.format("%s/forwarders/%s/events?page=%s&size=%s&sort=%s",
...
function inbox_forwarder_controller_api:get_inbox_forwarders(inbox_id, page, size, sort, search_filter, since, before)
	local req = http_request.new_from_uri({
		scheme = self.default_scheme;
		host = self.host;
		port = self.port;
		path = string.format("%s/forwarders?inboxId=%s&page=%s&size=%s&sort=%s&searchFilter=%s&since=%s&before=%s",
...
function inbox_forwarder_controller_api:update_inbox_forwarder(id, create_inbox_forwarder_options)
	local req = http_request.new_from_uri({
		scheme = self.default_scheme;
		host = self.host;
		port = self.port;
