...
function sent_emails_controller_api:get_sent_emails(inbox_id, page, size, sort)
	local req = http_request.new_from_uri({
		scheme = self.default_scheme;
		host = self.host;
		port = self.port;
		path = string.format("%s/sent?inboxId=%s&page=%s&size=%s&sort=%s",
...
function sent_emails_controller_api:get_sent_organization_emails(inbox_id, page, size, sort)
	local req = http_request.new_from_uri({
		scheme = self.default_scheme;
		host = self.host;
		port = self.port;
		path = string.format("%s/sent/organization?inboxId=%s&page=%s&size=%s&sort=%s",
			self.basePath, http_util.encodeURIComponent(inbox_id), http_util.encodeURIComponent(page), http_util.encodeURIComponent(size), http_util.encodeURIComponent(sort));
	})
